<script lang="ts">
    import RightArrowIcon from '$lib/icons/RightArrow.svelte';
    ...
    $: panelHeight = shouldShowResults ? (hasNoResults || hasError ? '100px' : Math.min(searchResults.length * 80 + 60, 400) + 'px') : '0px';
</script>
...
<main class="flex flex-col items-center justify-center h-[100%] px-4">
    ...
    <div class="mt-5 w-full max-w-md">
        ...
        <div 
            ...
            style="max-height: {panelHeight}; opacity: {shouldShowResults ? '1' : '0'};"
        >
            <div 
                ...
                style="max-height: {panelHeight}; transform: translateY({shouldShowResults ? '0' : '-10px'});"
            >
                {#if hasError}
